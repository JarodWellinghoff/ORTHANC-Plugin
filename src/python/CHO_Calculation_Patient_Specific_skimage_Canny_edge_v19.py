...
def prepare_Lesion_sig(
    ...
    *If psf_file is None* → a PSF is **simulated on a 50 mm FOV**
    ...
    if psf_file is None:  # ---- SIMULATED PSF ----
        ...
        wire_fov_mm = 50.0
        dx_psf = wire_fov_mm / PSF.shape[0]  # measured pixel size
...
mtf50 = (
    None  # Spatial frequency (cycles/mm) where MTF=0.5; replace with CT-specific value
)
mtf10 = None  # Spatial frequency (cycles/mm) where MTF=0.1; replace or set to None for Gaussian
...
location = np.linspace(0, Scan_len, num=len(CTDI_All))
