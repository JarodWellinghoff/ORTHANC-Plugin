...
class CTSlidingWindow:
    def __init__(
        ...
        window_unit: (
            str | None
        ) = None,  # 'cm' or 'slices' (overrides unit for window_length)
        ...
        if self.window_unit == "slices":
            if not isinstance(window_length, int) or window_length <= 0:
                raise ValueError(
                    "window_length must be a positive integer when window_unit='slices'"
                )
        else:  # cm
            if window_length <= 0:
        ...
        if self.step_unit == "slices":
            if not isinstance(step_size, int) or step_size <= 0:
                raise ValueError(
                    "step_size must be a positive integer when step_unit='slices'"
                )
        else:  # cm
            if step_size <= 0:
    ...
    def _find_slices_in_range(
        self, positions: np.ndarray, start_cm: float, end_cm: float
    ) -> List[int]:
    ...
    def _find_slices_by_index_range(
        self, start_idx: int, end_idx: int, total_slices: int
    ) -> List[int]:
    ...
    def _compute_slice(
        self, slice_idx: int, dicom_slices: List
        ...
        if (
            self.use_cache
            and slice_idx in self.pixel_array_cache
            and slice_idx in self.integral_edges_cache
            and slice_idx in self.integral_images_cache
            and slice_idx in self.integral_images_square_cache
        ):
            # if self.use_cache and slice_idx in self.pixel_array_cache:
            print(f"Using cached data for slice {slice_idx}")
            edges = feature.canny(
                self.pixel_array_cache[slice_idx].astype(float), sigma=self.sigma
            )
            return (
                self.pixel_array_cache[slice_idx],
                self.integral_edges_cache[slice_idx],
                self.integral_images_cache[slice_idx],
                self.integral_images_square_cache[slice_idx],
                edges,
            )
        ...
        if hasattr(dicom_slices[slice_idx], "RescaleSlope") and hasattr(
            dicom_slices[slice_idx], "RescaleIntercept"
        ):
        ...
        integral_edges = self._integral_image(
            edges, [self.padding_size, self.padding_size], "replicate"
        )
        integral_images = self._integral_image(
            pixel_array, [self.padding_size, self.padding_size], "replicate"
        )
        integral_images_square = self._integral_image(
            pixel_array**2, [self.padding_size, self.padding_size], "replicate"
        )

        pixel_array = pixel_array.astype(np.float32)
        integral_edges = integral_edges.astype(np.float32)
        integral_images = integral_images.astype(np.float32)
    ...
    def get_num_windows(self, dicom_slices: list) -> int:
        ...
        if self.window_unit == "cm" and self.step_unit == "cm":
            ...
            self.num_windows = (
                int((self.total_length - self.window_length) / self.step_size) + 1
            )
            ...
            if self.window_length > self.total_slices:
                ...
                self.num_windows = (
                    int((self.total_slices - self.window_length) / self.step_size) + 1
                )
            ...
            self.num_windows = self._count_mixed_windows_slice_window_cm_step(
                dicom_slices
            )
            ...
            self.num_windows = self._count_mixed_windows_cm_window_slice_step(
                dicom_slices
            )
    ...
    def _count_mixed_windows_cm_window_slice_step(self, dicom_slices: list) -> int:
        ...
        while current_slice_idx < self.total_slices:
            ...
            slice_indices = self._find_slices_in_range(
    ...
    def generate_windows(
        ...
        if (
            self.num_windows is None
            or self.positions is None
            or self.min_pos is None
            or self.max_pos is None
        ):
    ...
    def _generate_windows_cm_cm(
        ...
        for window_idx in range(self.num_windows):
            ...
            window_array, slice_positions = self._build_window(
                slice_indices, dicom_slices
            )
    ...
    def _generate_windows_slices_slices(
        self, dicom_slices: list
    ) -> Generator[Tuple[Dict[str, Any], Dict[str, Any]], None, None]:
        ...
        for window_idx in range(self.num_windows):
            ...
            slice_indices = self._find_slices_by_index_range(
                start_idx, end_idx, self.total_slices
            )
    ...
    def _generate_windows_slices_cm(
        self, dicom_slices: list
    ) -> Generator[Tuple[Dict[str, Any], Dict[str, Any]], None, None]:
        ...
        while window_idx < self.num_windows:
            ...
            slice_indices = list(
                range(start_slice_idx, start_slice_idx + self.window_length)
            )
    ...
    def _generate_windows_cm_slices(
        self, dicom_slices: list
    ) -> Generator[Tuple[Dict[str, Any], Dict[str, Any]], None, None]:
    ...
    def _create_metadata(
        ...
        metadata = {
            ...
            "window_length_cm": (
                abs(end_cm - start_cm) if len(slice_positions) > 1 else 0
            ),
            ...
            "relative_start_cm": (
                start_cm - self.min_pos if self.min_pos is not None else start_cm
            ),
            "start_idx": slice_indices[0] if slice_indices else 0,
            "end_idx": slice_indices[-1] if slice_indices else 0,
            "actual_end_cm": end_cm,
            "relative_end_cm": (
                end_cm - self.min_pos if self.min_pos is not None else end_cm
            ),
            "actual_mid_cm": (start_cm + end_cm) / 2,
            "relative_mid_cm": (
                (start_cm + end_cm) / 2 - self.min_pos
                if self.min_pos is not None
                else (start_cm + end_cm) / 2
            ),
            ...
            "actual_coverage_cm": (
                abs(slice_positions[-1] - slice_positions[0])
    ...
    def _build_window(
        self, slice_indices: List[int], dicom_slices: list
        ...
        for idx in slice_indices:
            (
                ...
                integral_images_square,
                edges,
            ) = self._compute_slice(idx, dicom_slices)
    ...
    def clear_cache(self, keep_indices: List[int] | None = None):
        ...
        if keep_indices is None:
            ...
            keys_to_remove = [
                k for k in self.pixel_array_cache.keys() if k not in keep_indices
            ]
            for k in keys_to_remove:
                del self.pixel_array_cache[k]

            keys_to_remove = [
                k for k in self.integral_edges_cache.keys() if k not in keep_indices
            ]
            for k in keys_to_remove:
                del self.integral_edges_cache[k]

            keys_to_remove = [
                k for k in self.integral_images_cache.keys() if k not in keep_indices
            ]
            for k in keys_to_remove:
            ...
            keys_to_remove = [
                k
                for k in self.integral_images_square_cache.keys()
...
def example_usage():
    ...
    window_gen_cm = CTSlidingWindow(
        window_length=15.0, step_size=5.0, unit="cm"  # 15 cm  # 5 cm step
    )

    # Example 2: Slice-based windows
    print("=== Slice-based windows ===")
    window_gen_slices = CTSlidingWindow(
        window_length=10, step_size=3, unit="slices"  # 10 slices  # 3 slice step
    )
    ...
    window_gen_compat = CTSlidingWindow(
        window_length=15.0, step_size=5.0, unit="cm"  # Both window and step will use cm
    )
...
if __name__ == "__main__":
    ...
    for name, config in configs.items():
        print(
            f"{name}: window={config.window_length}{config.window_unit}, "
