...
# ---- NEW (minimal additions for test runner support) ----
import json
import pathlib
...
def prepare_Lesion_sig(
    ...
    *If psf_file is None* → a PSF is **simulated on a 50 mm FOV**
    ...
    if psf_file is None:  # ---- SIMULATED PSF ----
        ...
        # dx_psf = wire_fov_mm / wire_Matrix_size
        dx_psf = wire_fov_mm / wire_Matrix_size / 4
        # Compute presampling MTF
        ...
        # ? Remove the /4 here?
        # scaling_factor = dx_psf / pixel_spacing_patient / 4
        scaling_factor = dx_psf / pixel_spacing_patient
        # ? Add a minimum size constraint here?
        scaling_factor = max(scaling_factor, 1 / PSF.shape[0])

        #! Scaling factor is very low on some patients, causes lesions to be empty
...
# ---------------------------------------------------------------------------
# Helper / plot functions (unchanged from original)
# ---------------------------------------------------------------------------
...
def plot_lesion_signals(
    ...
    idx = np.argmax(~(np.abs(np.gradient(np.abs(np.gradient(mtf)))) > 3e-5)) - 1
    fig, (ax1, ax2) = plt.subplots(
        1, 2, num=f"Presampling MTF - Lesion {i+1}", figsize=(12, 5)
    )
    show_freq = freq[:idx]
    show_mtf = mtf[:idx]
    if idx <= 10:  # sanity check to avoid very small idx
...
def main(
    ...
    dx = rFOV / Im_Size[0]
    dy = rFOV / Im_Size[1]
    patient_FOV = dcm_info.ReconstructionDiameter
    patient_matrix = dcm_info.Rows
    ...
    pixel_size = patient_FOV / patient_matrix
    wire_fov_mm = 50
    wire_Matrix_size = 512
    ...
    for mm in range(1, n_images // N_sub - 1):
        ...
        if NPS_Cal:
            ...
            nps_1D_sum_test += (NPS_1D_sum - nps_1D_sum_test) / nps_counter
            Total_NPS_Num = 200
            NPS_Cal = False
        del STD_all
        gc.collect()
    ...
    dps_location_array = np.reshape(dps_location_array, (n_images // N_sub - 2, -1))
    ...
    plot_noise_with_image(
        dps_location_array, Noise_level_local, coronal_image, slice_location
    )
    plot_detectability_with_image(
        dps_location_array, Mean_loc_dps, coronal_image, slice_location
    )
...
# ---------------------------------------------------------------------------
# ---- Entry point (unchanged behaviour when run directly) -----------------
# ---------------------------------------------------------------------------

if __name__ == "__main__":
    import argparse

    # Kept here so the script still works stand-alone with the original
    # hard-coded defaults, OR with explicit CLI overrides.
    Map_dir = r"\\mfad.mfroot.org\rchapp\eb028591\CT_CIC_Group_Server\Staff_Folders\Zhou_Zhongxing\Zhou_ZX"
    _default_dicom = Map_dir + "\\For_Jarod\\L067_FD_1_0_B30F_0001\\"
    _default_lesion = (
        Map_dir + "\\For_Jarod\\Liver_lesion_sample\\Patient02-411-920_Lesion1.mat"
    )
    _default_psf = Map_dir + "\\For_Jarod\\Liver_lesion_sample\\EID_PSF_Br44.mat"

    parser = argparse.ArgumentParser(description="CT Image Quality Pipeline")
    parser.add_argument("--dicom_dir", default=_default_dicom, help="DICOM directory")
    parser.add_argument(
        "--lesion_file", default=_default_lesion, help="Lesion .mat file"
    )
    parser.add_argument(
        "--psf_file", default=_default_psf, help="PSF .mat file (omit to simulate)"
    )
    parser.add_argument("--mtf50", type=float, default=0.434, help="MTF50 (cycles/mm)")
    parser.add_argument(
        "--mtf10",
        type=float,
        default=0.730,
        help="MTF10 (cycles/mm); omit for Gaussian",
    )
    ...
    main(
        ...
        psf_file=args.psf_file if args.psf_file else None,
        mtf50=args.mtf50,
        mtf10=args.mtf10,
