...
for i in range(len(Lesion_Contrasts)):
    ...
    Lesion_sig, freq, mtf = prepare_Lesion_sig(
        ...
        model=model,
        kernel=kernel,
        lesion_con_target=lesion_con_target,
...
if body_part is not None:
    ...
    if "ABDOMEN" in body_part_str or "PELVI" in body_part_str:
        ...
        para_a = 3.704369
        para_b = 0.03671937
        print(f"Unknown Body Part '{body_part_str}'. Defaulting to abdomen parameters.")
else:
...
# Plot NPS (after 2× oversampling)
fig, ax = plt.subplots(figsize=(8, 6))
ax.plot(Spatial_freq, NPS_1D, "b", linewidth=2, label="average (2 x oversampled)")
ax.set_xlabel("Spatial frequency (cycles/cm)", fontsize=16)
ax.set_ylabel("NPS (HU² cm²)", fontsize=16)
ax.set_title("Noise Power Spectrum (2 x oversampled ROIs)", fontsize=14)
